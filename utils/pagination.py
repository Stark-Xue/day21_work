...
class Page:
    ...
    def page_str(self):
        ...
        if self.current_page - 1 < 1:
            prev = "<a class='page' href='/application/?p=1'>上一页</a>"
        else:
            prev = "<a class='page' href='/application/?p=%s'>上一页</a>" % str(self.current_page - 1)
        page_list.append(prev)
        for i in range(int(start_index), int(end_index)):
            if self.current_page == i:
                temp = "<a class='page active' href='/application/?p=%s'>%s</a>" % (i, i)
            else:
                temp = "<a class='page' href='/application/?p=%s'>%s</a>" % (i, i)
            page_list.append(temp)

        if self.current_page + 1 > self.total_page_count:
            nex = "<a class='page' href='/application/?p=%s'>下一页</a>" % self.total_page_count
        else:
            nex = "<a class='page' href='/application/?p=%s'>下一页</a>" % str(self.current_page + 1)
        page_list.append(nex)

        jump = """<input type='text' /> <a onclick="jumpTo(this, '/application/?p=');">Go</a>
                    <script>
